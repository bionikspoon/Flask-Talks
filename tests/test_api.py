...
class CommentModelTestCase(unittest.TestCase):
    ...
    def test_token_errors(self):
        ...
        c = Comment(talk=t, body='c1', author_name='n', author_email='e@e.com',
                    approved=False)
        db.session.add_all([u1, u2, t, c])
        db.session.commit()

        # missing JSON --> 400
        with self.app.test_request_context(API.format(c.id), method='PUT'):
        ...
        # bad token --> 401
        bad_token_dict = {'token': 'a bad token'}
        with self.app.test_request_context(API.format(c.id), method='PUT',
                                           data=json.dumps(bad_token_dict),
                                           headers=JSON_HEADER):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 401)
    ...
    def test_approve(self):
        ...
        with self.app.test_request_context(API.format(c.id), method='DELETE',
                                           data=json.dumps({'token': token}),
                                           headers=JSON_HEADER):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 400)
